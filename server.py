...
def _run_pipeline(params: dict) -> dict:
    ...
    # ── QuantumAgent: solve the final QUBO ───────────────────────────────────
    ...
    def _quantum_rationale(selected: list[str]) -> tuple[dict, float]:
        ...
        caveats = []
        if qubo_result.get("backend", "").startswith("greedy"):
            caveats.append("Universe too large for exact QUBO solve — greedy heuristic used")
        elif not XPYQ_KEY:
            caveats.append("QUBO solved by local brute force (no xpyq key configured)")
...
def api_ibm_submit():
    if not IBM_TOKEN:
        return jsonify({"error": "IBM_TOKEN not configured on the server."}), 400
    return jsonify({"error": "IBM QPU integration not yet implemented. Configure IBM_TOKEN and install qiskit-ibm-runtime."}), 501


@app.route("/api/quantum/ibm/status", methods=["POST"])
def api_ibm_status():
    return jsonify({"error": "IBM QPU integration not yet implemented."}), 501
